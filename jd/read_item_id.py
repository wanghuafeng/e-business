...
CONFIG_PATH = os.path.dirname(PATH)# the path you can read the config file
...
def get_id_from_breakpoint():
    ...
    items_id_filename = os.path.join(PATH, 'page_url', item_id_filename)
...
def read_failed_id():
    ...
    re_failed_id_filename = os.path.join(PATH, 'log', 're_failed_item_id')
    crawled_failed_id = os.path.join(PATH, 'log', 're_crawled_item_id')
    with codecs.open(failed_id_filename, encoding='utf-8') as failed_id_f,\
    codecs.open(crawled_failed_id, mode='wb', encoding='utf-8') as crawled_id_wf,\
    codecs.open(re_failed_id_filename, mode='wb', encoding='utf-8') as re_failed_id_wf:
