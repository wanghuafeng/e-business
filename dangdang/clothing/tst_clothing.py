...
def get_root_page_url():
    url = 'http://category.dangdang.com/cid4002742.htm'
...
def read_one_page_url_to_get_pagesize():
    url = 'http://category.dangdang.com/cid4008149.html'
    html = urllib2.urlopen(url).read()
    page_size = re.search(r'<span class="left"></span>.*?/(\d+)</span>', html).group(1)
    print page_size
...
def read_one_page_url_get_item_id():
    ip_port = '122.96.59.99:80'
    http_handler = urllib2.ProxyHandler({'http':'http://%s'%ip_port})
    opener = urllib2.build_opener(http_handler)
    urllib2.install_opener(opener)
    url = 'http://category.dangdang.com/cid4008149-pg21.html'
    html = urllib2.urlopen(url).read()
    soup = BeautifulSoup(html, 'html5lib')
    div_level_str = soup.find('div', class_='shoplist')
    li_level_list = div_level_str.find_all('li', attrs={'name':'lb'})
    url_str_list = [item.a['href'] for item in li_level_list]
    url_list = [re.search(r'http://product.dangdang.com/\d+\.html', item).group() for item in url_str_list]
    print url_list
...
def get_item_id():
    ...
    item_id_url = os.path.join(PATH, 'sys', 'clothing_item_id')
