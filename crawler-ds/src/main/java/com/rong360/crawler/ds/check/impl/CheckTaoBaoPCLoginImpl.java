...
import com.rong360.crawler.ds.check.ICheckTaoBaoPCLogin;
 ...
 * 
 */
public class CheckTaoBaoPCLoginImpl implements ICheckTaoBaoPCLogin {

	@Override
	public CheckResult check(TaoBaoPCLoginQuery taoBaoPCLoginQuery) {
		
		/******检查登录名为空*****/
		if (StringUtils.isEmpty(taoBaoPCLoginQuery.getLoginName())) {
			return new CheckResult(ErrorCode._20001.getMsg(), ErrorCode._20001.getCode());
		}
		
		/******检查密码是否为空*****/
		if (StringUtils.isEmpty(taoBaoPCLoginQuery.getPassword())) {
			return new CheckResult(ErrorCode._20002.getMsg(), ErrorCode._20002.getCode());
		}
		
		/******检查userId是否为空*****/
		if (isEmpty(taoBaoPCLoginQuery.getUserId())) {
